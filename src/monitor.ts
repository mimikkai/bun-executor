...
function sh(cmd: string, args: string[], timeout = 5000): Promise<{ ok: boolean; out: string }> {
...
function shFile(file: string, args: string[], timeout = 5000): Promise<{ ok: boolean; out: string }> {
  const { promise, resolve } = Promise.withResolvers<{ ok: boolean; out: string }>();
...
function bunBinaryCandidates(): string[] {
...
async function freshInstallPresent(): Promise<{ path: string; version: string } | null> {
  for (const f of bunBinaryCandidates()) {
    if (!fs.existsSync(f)) continue;
    const r = await shFile(f, ["--version"]);
    if (r.ok && /^\d/.test(r.out.trim())) return { path: f, version: r.out.trim() };
  }
  return null;
}
...
async function detectBun(): Promise<BunInfo> {
...
function listBunProcesses(): Promise<BunProcess[]> {
  const { promise, resolve } = Promise.withResolvers<BunProcess[]>();
...
function killBunProcess(pid: number, refresh: () => void): void {
...
class Node {
  constructor(
    public type: "status" | "info" | "process",
    public label: string,
...
class BunMonitorProvider implements vscode.TreeDataProvider<Node> {
  bun: BunInfo = { installed: false, version: null, path: null };
  procs: BunProcess[] = [];
...
function updateStatusBar(item: vscode.StatusBarItem, provider: BunMonitorProvider): void {
...
function promptModalReload(version: string): void {
  void vscode.window
    .showInformationMessage(
      `Bun ${version} was installed successfully. Reload VS Code so Bun appears on PATH.`,
      { modal: true },
      "Reload Window"
    )
    .then((pick) => {
...
async function ensureBun(refresh: () => void): Promise<void> {
  ...
  const w = vscode.window as typeof vscode.window & {
...
export function registerMonitor(context: vscode.ExtensionContext): void {
  ...
  context.subscriptions.push(
    tree,
    statusItem,
    vscode.commands.registerCommand("bunExecutor.installBun", () => ensureBun(refresh)),
    ...
    vscode.commands.registerCommand("bunExecutor.killProcess", (node: Node) => {
      if (node && node.type === "process" && node.pid) killBunProcess(node.pid, refresh);
    })
  );
